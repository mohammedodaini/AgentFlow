...
- **Phase:** **M1–M10 complete (2026-08-15).** `make check` green — ruff, ruff
  format, mypy strict over **221 files**, **596 tests**, **98.20% coverage**
  (gate at 97%). See
  [docs/milestones/M10-conversations-and-memory.md](docs/milestones/M10-conversations-and-memory.md).
...
- **Mode: autonomous through M12.** The user said, verbatim: *"let us do that
  ...
  otherwise.
- **Next: M11 (first OAuth integration — Google).** Build it autonomously; do
  not ask first. `docs/CONTINUE.md` carries the brief. The key risk named in
  the roadmap: storing a refresh token in plaintext. It is a credential to
