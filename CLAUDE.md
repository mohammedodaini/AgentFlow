...
- **Phase:** **M1–M11 complete (2026-08-16).** `make check` green — ruff, ruff
  format, mypy strict over **228 files**, **667 tests**, **97.77% coverage**
  (gate at 97%). See
  [docs/milestones/M11-first-oauth-integration.md](docs/milestones/M11-first-oauth-integration.md).
...
- **Mode: autonomous through M12.** The user said, verbatim: *"let us do that
  ...
  otherwise.
- **Next: M12 (human-in-the-loop approvals)** — the last milestone of the
  autonomous run. Build it autonomously; do not ask first. `docs/CONTINUE.md`
  carries the brief. It is where `agent_runs.checkpoint` and
  `RunStatus.PAUSED_FOR_APPROVAL` finally get used (both exist, unwritten,
  since M9), where `cost_usd` stops being `Decimal(0)`, and where the Google
  Calendar **write** scope is finally earned. **Mentor mode resumes at M13.**
